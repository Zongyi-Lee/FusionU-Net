...
def get_model(model_name):
    ...
    if 'FusionUNet' in model_name:
        from funet import FusionUNet
        return FusionUNet(3, config.n_labels, 64, 3)

    elif 'UCTransNet' in model_name:
        from nets.UCTransNet.UCTransNet import UCTransNet, UCTransNet_type2
        config_vit = config.get_CTranS_config()        
        if 'type2' in model_name:
            return UCTransNet_type2(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        return UCTransNet(config_vit,n_channels=config.n_channels,n_classes=config.n_labels)
    
    elif model_name == 'TransUNet':
        from nets.trainsunet.vit_seg_modeling import CONFIGS as transconfig
        from nets.trainsunet.vit_seg_modeling import VisionTransformer as VitSeg

        config_vit = transconfig['R50-ViT-B_16']
        config_vit.n_classes = config.n_labels
        config_vit.n_skip = 3
        vit_patches_size = 16
        config_vit.patches.grid = (int(config.img_size / vit_patches_size), int(config.img_size / vit_patches_size))
        return VitSeg(config_vit, img_size=config.img_size, num_classes=config.n_labels)
    elif model_name == 'SwinUNet':
        from ml_collections import ConfigDict
        from nets.swinunet.vision_transformer import SwinUnet
        from yacs.config import CfgNode as CN
        from nets.swinunet.config import get_config
        swin_config = ConfigDict()
        swin_config.cfg = './nets/swinunet/swin_tiny_patch4_window7_224_lite.yaml'
        swin_config.opts = None
        swin_config.batch_size = config.batch_size
        swin_config.zip = False
        swin_config.cache_mode = 'part'
        swin_config.resume = False
        swin_config.accumulation_steps = False
        swin_config.use_checkpoint = False
        swin_config.amp_opt_level = 'O1'
        swin_config.tag = False
        swin_config.eval = False
        swin_config.throughput = False
        
        swin_cfg =  get_config(swin_config)

        return SwinUnet(swin_cfg, 
                        img_size=config.img_size,
                        num_classes=config.n_labels)
    elif model_name == 'UNet++':
        from nets.unetpp.unetpp import NestedUNet
        return NestedUNet(num_classes=config.n_labels)
    elif model_name == 'UNet':
        from nets.UNet import UNet
        return UNet(n_channels=3, n_classes=1)
    elif model_name == 'ReshapeUNet':
        from nets.UNet import ReshapeUNet
        return ReshapeUNet(3, 1)
    else: 
